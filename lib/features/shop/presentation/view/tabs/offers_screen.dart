...
class _OffersScreenState extends State<OffersScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<OffersCubit, OffersState>(
      ...
      builder: (context, state) {
        ...
        if (state is OffersLoading || (state is OffersInitial && offers.isEmpty)) {
        ...
        return ValueListenableBuilder<String>(
          ...
          builder: (context, searchQuery, _) {
            ...
            final filteredOffers = searchQuery.isEmpty
                ? offers
                : offers.where((o) {
            ...
            return ValueListenableBuilder<int>(
              ...
              builder: (context, currentPage, _) {
                // Pagination logic
                final totalPages = filteredOffers.isEmpty 
                    ? 1 
                ...
                final currentOffers = filteredOffers.isEmpty
                    ? <Content>[]
                    : filteredOffers.sublist(
                        startIndex,
                        endIndex,
                      );
                        ...
                        children: [
                          SingleChildScrollView(
                            ...
                            child: DataTable(
                              ...
                              rows: currentOffers
                                  .map((offer) => _buildOfferRow(context, offer, local))
                                  .toList(),
                            ),
                          ),
                          // Pagination
  ...
  DataRow _buildOfferRow(BuildContext context, Content offer, AppLocalizations local) {
    Color statusColor;
    ...
    String discount = offer.discountValue != null ? offer.discountValue.toString() : "-";
    if (offer.discountType != null && offer.discountType is String && offer.discountType!.isNotEmpty) {
          ...
          Row(
            children: [
              IconButton(
                ...
                tooltip: 'Edit',
                onPressed: () => _showAddOrEditDialog(context, AppLocalizations.of(context)!, offer: offer),
              ),
                ...
                onPressed: () {
                  showDialog(
                    ...
                    builder: (ctx) => AlertDialog(
                      title: Text(local.delete,style: TextStyle(color: AppColors.primary),),
                      content: Text(local.are_you_sure_delete),
                      actions: [
                        TextButton(
                          onPressed: () => Navigator.pop(ctx),
                          child: Text(local.cancel,style: TextStyle(color: AppColors.primary)),
                        ),
                        TextButton(
                          onPressed: () {
                            Navigator.pop(ctx);
  ...
  Future<void> _showAddOrEditDialog(BuildContext context, AppLocalizations local, {Content? offer}) async {
    ...
    final discountController = TextEditingController(text: offer?.discountValue?.toString() ?? '');
    final startDateController = TextEditingController(
      text: offer?.startDate != null 
        ? DateTime.tryParse(offer!.startDate!)?.toLocal().toIso8601String().split('T').first ?? ''
        : ''
    );
    final endDateController = TextEditingController(
      text: offer?.endDate != null 
        ? DateTime.tryParse(offer!.endDate!)?.toLocal().toIso8601String().split('T').first ?? ''
        : ''
    );
    ...
    final outerContext = context; // Capture the outer context that has access to OffersCubit
                  ...
                  GestureDetector(
                    onTap: () async {
                      ...
                      if (picked != null) {
                        setDialogState(() {
                          startDate = picked;
                          startDateController.text = picked.toLocal().toIso8601String().split('T').first;
                        });
                      }
                    },
                    child: AbsorbPointer(
                  ...
                  GestureDetector(
                    onTap: () async {
                      ...
                      if (picked != null) {
                        setDialogState(() {
                          endDate = picked;
                          endDateController.text = picked.toLocal().toIso8601String().split('T').first;
                        });
                      }
                    },
                    child: AbsorbPointer(
            ...
            CustomElevatedButton(
              ...
              onPressed: () {
                ...
                final req = OfferRequest(
                  ...
                  discountValue: double.tryParse(discountController.text.trim()),
                ...
                if (offer != null && offer.id != null) {
                  outerContext.read<OffersCubit>().updateOffer(offer.id!, req);
                } else {
